...
---
### #27 — 기획문서 v2 2차 반영: 결정 6건 🔵진행 중(1~3단계 완료)
...
- **상태**: 🔵 1~3단계 완료(빌드 그린 · `passageTests` 전체 통과). 시뮬레이터 실측으로 확인한 것 — 세션 종료 화면 순서(페이지→장소→지도→장소 태그→사진)·히어로 170pt·완독 책 단일 CTA·% 제거·각진 표지·채도 0.66 톤·필터 권수(모든 책 3/읽는 중 2/완독 1). **미확인**: ⋮ 메뉴 완독 토글과 인라인 페이지 입력(각각 탭 타깃이 작고, 페이지 수 없는 책이 필요해 자동화로 도달하지 못함) → 실기 확인 권장. 4단계는 착수 전.
